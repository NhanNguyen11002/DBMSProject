...
namespace QLRauMaVer1.Forms
{
    public partial class DonHang : Form
    {
        ...
        {
            dtgQLDH.DataSource = donhang.SelectAllDonHangByShopId(shop_id);
            shopidTextbox.Text = shop_id;
        }
        ...
        private void paymentGrbox_Enter(object sender, EventArgs e)
        {

        }

        private void voucherCbbox_SelectedIndexChanged(object sender, EventArgs e)
        {
            VoucherDao vc = new VoucherDao();
            string voucher_id = voucherCbbox.SelectedItem.ToString();
            float discount = float.Parse(vc.getDiscount(voucher_id));
            totalpriceTextbox.Text = (Int32.Parse(totalpriceTextbox.Text.ToString())*(1-discount)).ToString();
        }
    }
}
